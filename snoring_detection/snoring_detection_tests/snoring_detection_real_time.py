...
# Define constants
MODEL_PATH = '../models/cnn_16k.keras'  # Replace with the path to your trained model
...
def classify_snoring_segment(segment, original_sample_rate):
    """Classifies snoring events in the given audio segment."""
    # 1) Downsample from the original sample rate (44.1kHz) to 16kHz
    desired_sr = 16000
    segment_16k = resample_poly(segment, desired_sr, original_sample_rate)

    # 2) Convert to tensor and normalize
    x = tf.convert_to_tensor(segment_16k, dtype=tf.float32)
    x = x / (tf.reduce_max(tf.abs(x)) + 1e-6)  # Normalize to -1..1
    # Ensure x is 1D
    x = tf.reshape(x, [-1])

    # 3) Get log-mel spectrogram at 16kHz
    x = get_log_mel(x, desired_sr)
...
class SnoreDetectorGUI:
    def __init__(self, root):
        ...
        self.sample_rate = 44100  # Record at 44.1kHz
    ...
    def detect_snoring(self):
        ...
        try:
            while self.is_running:
                # Record audio segment at 44.1kHz
                segment = sd.rec(self.samples_per_segment,
                ...
                # Classify the segment (includes downsampling to 16kHz)
